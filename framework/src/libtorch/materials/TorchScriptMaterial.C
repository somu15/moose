...
TorchScriptMaterial::validParams()
{
  ...
  params.addRequiredParam<UserObjectName>("gp_inadequacy_term", "The Gaussian process torch script to account for model inadequacy.");
  return params;
}
...
TorchScriptMaterial::TorchScriptMaterial(
  ...
  : Material(parameters),
    ...
    _gp_inadequacy_term(getUserObject<TorchScriptUserObject>("gp_inadequacy_term")),
...
TorchScriptMaterial::computeQpValues()
{
  ...
  std::vector<Real> input_vector_gp(4, 0.0);
  input_vector_gp[0] = -1.1810;
  input_vector_gp[1] = -0.1275;
  torch::Tensor input_tensor_gp;
  LibtorchUtils::vectorToTensor(input_vector_gp, input_tensor_gp);
  input_tensor_gp = input_tensor_gp.transpose(0, 1);
  // std::cout << Moose::stringify(input_tensor_gp) << std::endl;
  auto gp_term = _gp_inadequacy_term.evaluate(input_tensor_gp);
  std::cout << Moose::stringify(gp_term) << std::endl;
