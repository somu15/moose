...
class AL_ADAM_Parallel : public ActiveLearningReporterTempl<Real>, public CovarianceInterface
{
public:
  static InputParameters validParams();
  AL_ADAM_Parallel(const InputParameters & parameters);
...
protected:
  ...
  // PetscErrorCode FormInitialGuess(AL_ADAM_Parallel * GP_ptr, Vec theta);
...
template <typename T>
T &
AL_ADAM_Parallel::declareModelData(const std::string & data_name)
{
...
template <typename T>
T &
AL_ADAM_Parallel::declareModelData(const std::string & data_name, const T & value)
{
  RestartableData<T> & data_ref = declareModelDataHelper<T>(data_name);
  data_ref.set() = value;
  return data_ref.set();
...
template <typename T>
RestartableData<T> &
AL_ADAM_Parallel::declareModelDataHelper(const std::string & data_name)
{
