...
class AMCP_3 : public GeneralReporter
{
public:
  static InputParameters validParams();
  AMCP_3(const InputParameters & parameters);
...
protected:
  ...
  template <typename T>
  std::vector<T *> declareAMCP_3Values(const std::string & prefix);
  // template <typename T>
  // std::vector<std::vector<T *>> declareAMCP_3Values(const std::string & prefix); // , const dof_id_type & num_rows
  template <typename T>
  std::vector<std::vector<T> *> declareAMCP_3VectorValues(const std::string & prefix);
  ///@}
...
template <typename T>
InputParameters
AMCP_3::addReporterTypeParams(const std::string & prefix, bool add_vector)
{
...
template <typename T>
std::vector<T *>
AMCP_3::declareAMCP_3Values(const std::string & prefix)
{
