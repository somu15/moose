...
class AMCP_clean : public GeneralReporter
{
public:
  static InputParameters validParams();
  AMCP_clean(const InputParameters & parameters);
...
protected:
  ...
  template <typename T>
  std::vector<T *> declareAMCP_cleanValues(const std::string & prefix);
  // template <typename T>
  // std::vector<std::vector<T *>> declareAMCP_cleanValues(const std::string & prefix); // , const dof_id_type & num_rows
  template <typename T>
  std::vector<std::vector<T> *> declareAMCP_cleanVectorValues(const std::string & prefix);
  ///@}
...
template <typename T>
InputParameters
AMCP_clean::addReporterTypeParams(const std::string & prefix, bool add_vector)
{
...
// AMCP_clean::declareAMCP_cleanValues(const std::string & prefix, const dof_id_type & num_rows)
// {
...
template <typename T>
std::vector<T *>
AMCP_clean::declareAMCP_cleanValues(const std::string & prefix)
{
  std::string names_param(prefix + "_names");
  std::string values_param(prefix + "_values");
  std::vector<T *> data;
...
// template <typename T>
// std::vector<std::vector<T> *>
// AMCP_clean::declareConstantVectorReporterValues(const std::string & prefix)
// {
//   return this->declareAMCP_cleanValues<std::vector<T>>(prefix + "_vector");
// }
