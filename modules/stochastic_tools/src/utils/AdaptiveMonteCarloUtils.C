...
Real
AdaptiveMonteCarloUtils::computeSTD(const std::vector<Real> & data, const unsigned int & uselog)
{
  ...
  {
    if (uselog == 1)
      sq_diff1 += std::pow((std::log(data[i])-sum1/(data.size()-2)), 2);
    else
...
std::vector<Real>
AdaptiveMonteCarloUtils::computeABS(const std::vector<Real> & data)
{
