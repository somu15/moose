    ...
    for (dof_id_type k = _sampler->getLocalRowBegin(); k < _sampler->getLocalRowEnd(); ++k)
    {
        ...
        } else
        {
          ...
          if (_subset > (std::floor((_step-2) /  _sampler->parameters().get<int>("num_samplessub"))))
          {
            _ind_sto = -1;
            _count = INT_MAX;
            _output_sorted[k] = sortOUTPUT(_outputs_sto[k], _sampler->parameters().get<int>("num_samplessub"), _subset, _sampler->parameters().get<Real>("subset_probability"));
            for (dof_id_type j = 0; j < _sampler->parameters().get<std::vector<DistributionName>>("distributions").size(); ++j)
            {
              ...
              _inputs_sorted[j][k] = sortINPUT(_inputs_sto[j][k], _outputs_sto[k], _sampler->parameters().get<int>("num_samplessub"), _subset, _sampler->parameters().get<Real>("subset_probability"));
            }
            _output_limits.push_back(computeMIN(_output_sorted[k]));
          }
          if (_count >= _count_max)
          {
            ++_ind_sto;
