...
//   // std::cout << "Here 1 ********" << _output_value.size() << std::endl;
//   // Check whether the number of likelihoods match the number of outputs per subApp
//   if (_likelihoods.size() != 1) // _output_value[0].size()
//     mooseError("The number of likelihoods and outputs per subApp should be equal.");
...
//   // Resizing the data arrays to transmit to the output file
//   _inputs.resize(_props, std::vector<Real>(_sampler.getNumberOfCols() - _num_confg_params, 0.0));
//   // _inputs.resize(_props);
//   // for (unsigned int i = 0; i < _props; ++i)
//   //   _inputs[i].resize(_sampler.getNumberOfCols() - _num_confg_params);
//   // _outputs_required.resize(_sampler.getNumberOfRows());
//   _outputs_required.resize(_sampler.getNumberOfRows(), std::vector<Real>(_likelihoods.size(), 0.0));
//   _tpm.resize(_props);
//   _variance.resize(_props);
// }
...
//                                                        DenseMatrix<Real> & inputs_matrix)
// {
//   std::vector<std::vector<Real>> out1(_likelihoods.size(),
//                                       std::vector<Real>(_num_confg_values, 0.0));
//   std::vector<std::vector<Real>> out2(_likelihoods.size(),
//                                       std::vector<Real>(_num_confg_values, 0.0));
//   for (unsigned int i = 0; i < evidence.size(); ++i)
//   {
//     evidence[i] = 0.0;
...
//       evidence[i] += (std::log(_priors[j]->pdf(inputs_matrix(i, j))) -
//                       std::log(_priors[j]->pdf(_data_prev(i, j))));
//     for (unsigned int k = 0; k < _likelihoods.size(); ++k)
//     {
//       for (unsigned int j = 0; j < _num_confg_values; ++j)
//       {
//         out1[k][j] = _outputs_required[j * _props + i][k];
//         out2[k][j] = _outputs_prev[j * _props + i][k];
//       }
//     }
//     if (_var_prior)
//     {
...
//     }
//     for (unsigned int j = 0; j < _likelihoods.size(); ++j)
//       evidence[i] += (_likelihoods[j]->function(out1[j]) - _likelihoods[j]->function(out2[j]));
//   }
// }
