...
{
  Real mu1 = AdaptiveMonteCarloUtils::computeMean(ref, 0);
  Real std1 = AdaptiveMonteCarloUtils::computeSTD(ref, 0);
  ...
  for (int i = 0; i < _step-1; ++i)
  {
    for (int j = 0; j < _step-1; ++j)
    {
      ...
      for (unsigned int k = 0; k < inputs_norm.size(); ++k)
        tmp += std::exp(std::pow((inputs_norm[k][j] - inputs_norm[k][i]), 2));
      psi(i,j) = tmp;
    }
  }
...
void ActiveLearningBayesGaussBasis::computePrediction(const std::vector<Real> & inputs, const Real & userSig)
{
  ...
  for (unsigned int j = 0; j < _muP.size(); ++j)
  {
    Real tmp = 0.0;
    for (unsigned int k = 0; k < inputs.size(); ++k)
      tmp += std::exp(std::pow((inputs[k] - inputs_norm[k][j]), 2));
    psi_pred(j) = tmp;
  }
                                              ...
                                              Real & val)
{
  ...
  } else if (_step == N)
  {
    ...
    // computePrediction(row, 1.0);
    if (_SigPredict / _muPredict <= 0.05)
    {
      // _decision = true;
      _decision = false;
