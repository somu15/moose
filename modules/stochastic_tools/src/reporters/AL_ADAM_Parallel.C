...
registerMooseObject("StochasticToolsApp", AL_ADAM_Parallel);

InputParameters
AL_ADAM_Parallel::validParams()
{
...
AL_ADAM_Parallel::AL_ADAM_Parallel(
...
void
AL_ADAM_Parallel::SetupData(const std::vector<std::vector<Real>> & inputs, const std::vector<Real> & outputs)
{
...
void
AL_ADAM_Parallel::Train_ADAM(const unsigned int & iters)
{
...
void
AL_ADAM_Parallel::mapToVec(libMesh::PetscVector<Number> & theta) const
{
  for (auto iter = _tuning_data.begin(); iter != _tuning_data.end(); ++iter)
  {
    std::string hyper_param_name = iter->first;
...
void
AL_ADAM_Parallel::vecToMap(libMesh::PetscVector<Number> & theta)
{
  for (auto iter = _tuning_data.begin(); iter != _tuning_data.end(); ++iter)
  {
    std::string hyper_param_name = iter->first;
...
std::vector<Real>
AL_ADAM_Parallel::Predict_ADAM(const std::vector<Real> & inputs)
{
...
// bool
// AL_ADAM_Parallel::needSample(const std::vector<Real> & row,
//                                               dof_id_type,
//                                               dof_id_type,
//                                               Real & val)
// {
                                              ...
                                              Real & val)
{
  if (_step <= _N_train)
  {
    ...
    _communicator.allgather(_inputs_sto);
    std::cerr << "Inputs 1 " << Moose::stringify(_inputs_sto[0]) << std::endl;
    }
    if (_step == _N_train)
    {
      if (local_ind == 0)
