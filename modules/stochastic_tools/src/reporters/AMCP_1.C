...
AMCP_1::validParams()
{
 ...
 params.addRequiredParam<ReporterName>(
     "ipyc_output", "The name of the interger data"); // MooseDocs:data
 ...
 params.addParam<ReporterValueName>(
     "ipyc_data", "ipyc_data", "The name of the interger data"); // MooseDocs:data
  // params.addRequiredParam<ReporterName>(
...
AMCP_1::AMCP_1(const InputParameters & parameters)
  : GeneralReporter(parameters),
    ...
    _ipyc_output(&getReporterValue<std::vector<Real>>("ipyc_output", REPORTER_MODE_DISTRIBUTED)),
    ...
    _ipyc_data(declareValueByName<std::vector<Real>, ReporterGatherContext>("ipyc_data")),
...
AMCP_1::execute()
{
  if (_sampler->parameters().get<std::string>("_type") == "SSP_1")
  {
    if (_step <= (_sampler->parameters().get<int>("num_samplessub") / n_processors()))
    {
      ...
      _ipyc_data = (*_ipyc_output);
      _communicator.allgather(_ipyc_data);
    ...
    } else
    {
      ...
      std::cerr << n_processors() << std::endl;
    }
