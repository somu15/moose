...
ExtremeValue::validParams()
{
  ...
  params.addClassDescription(
      "Generalized extreme value likelihood function evaluating the model goodness against experiments.");
  // params.addRequiredParam<Real>("shape", "The shape parameter.");
  return params;
}

ExtremeValue::ExtremeValue(const InputParameters & parameters)
  : Gaussian(parameters) // , _shape(getParam<Real>("shape"))
{
}
