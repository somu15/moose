...
registerMooseObjectAliased("StochasticToolsApp", AMCP_clean, "AMCP_clean");
...
AMCP_clean::AMCP_clean(const InputParameters & parameters)
  : GeneralReporter(parameters),
    // _output(declareAMCP_cleanValues<Real>("output")),
    ...
    _step(getCheckedPointerParam<FEProblemBase *>("_fe_problem_base")->timeStep())
{
  ...
  // _output = declareAMCP_cleanValues<Real>("output", _sampler->parameters().get<dof_id_type>("num_rows"));
  // _inputs = declareAMCP_cleanValues<Real>("inputs", _sampler->parameters().get<dof_id_type>("num_rows"));
  ...
  if (_sampler->parameters().get<std::string>("_type") == "SSP_clean")
  {
