...
#include "StretchDecision.h"
#include "Sampler.h"
#include "DenseMatrix.h"

registerMooseObject("StochasticToolsApp", StretchDecision);

InputParameters
StretchDecision::validParams()
{
...
StretchDecision::StretchDecision(const InputParameters & parameters)
...
void
StretchDecision::computeEvidence(std::vector<Real> & evidence, DenseMatrix<Real> & inputs_matrix)
{
  std::vector<Real> out1(_num_confg_values);
  std::vector<Real> out2(_num_confg_values);

  std::vector<Real> out3(_num_confg_values);
  std::vector<Real> out4(_num_confg_values);
  for (unsigned int i = 0; i < evidence.size(); ++i)
  {
    ...
    for (unsigned int j = 0; j < _num_confg_values; ++j)
    {
      ...
      out3[j] = _outputs_required1[j * _props + i];
      out4[j] = _outputs_prev1[j * _props + i];
    }
    if (_var_prior)
    {
      ...
      evidence[i] += _likelihoods[1]->function(out3);
      _noise = std::sqrt(_var_prev[i]);
      evidence[i] -= _likelihoods[0]->function(out2);
      evidence[i] -= _likelihoods[1]->function(out4);
    }
    else
    {
      evidence[i] += (_likelihoods[0]->function(out1) - _likelihoods[0]->function(out2));
      evidence[i] += (_likelihoods[1]->function(out3) - _likelihoods[1]->function(out4));
    }
...
void
StretchDecision::computeTransitionVector(std::vector<Real> & tv, std::vector<Real> & evidence)
{
...
void
StretchDecision::nextSamples(std::vector<Real> & req_inputs,
