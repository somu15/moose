...
//           _inputs_sto[k].push_back(_inputs_prev[ss][k]); // _inputs_sto[k].push_back(data_in(ss,k));
//       }
//     }
//     if (_step == _n_train)
//     {
...
//       {
//         for (unsigned int j = 0; j < _sampler.getNumberOfCols(); ++j)
//           _inputs[ss][j] = data_in(ss, j); // _inputs_prev[ss]; // row;
...
//   } else
//   {
//     if (_track_gp_fails >= _allowed_gp_fails) // _decision[local_ind] == true && _flag_sample[local_ind] == true
//     {
//       for (dof_id_type ss = 0; ss < _sampler.getNumberOfRows(); ++ss)
//       {
//         _outputs_sto.push_back(output1[ss]);
...
//   }
//   return _decision[local_ind]; // 0; // 
// }

// void
// ActiveLearningGPDecision::initialize()
// {
// }
