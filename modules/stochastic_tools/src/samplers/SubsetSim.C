...
registerMooseObjectAliased("StochasticToolsApp", SubsetSim, "SubsetSim");
...
SubsetSim::SubsetSim(const InputParameters & parameters)
...
std::vector<Real>
SubsetSim::sortOUTPUT(const std::vector<Real> & outputs, const int & samplessub, const unsigned int & subset, const Real & subset_prob)
{
  std::vector<Real> tmp;
...
std::vector<Real>
SubsetSim::sortINPUT(const std::vector<Real> & inputs, const std::vector<Real> & outputs, const int & samplessub, const unsigned int & subset, const Real & subset_prob)
{
  std::vector<Real> tmp;
...
Real
SubsetSim::computeMIN(const std::vector<Real> & data)
{
  Real tmp = data[0];
...
// Real
// SubsetSim::computeSample(dof_id_type row_index, dof_id_type col_index)
// {
//   TIME_SECTION(_perf_compute_sample);
//   dof_id_type offset = _values_distributed ? getLocalRowBegin() : 0;
//   if (_step > 2 && _step <= _num_samplessub)
//   {
//     _subset = std::floor(_step / _num_samplessub);
//     if (col_index == 0 && std::abs(_values_ptr[0][row_index-offset]) != _outputs_sto[_outputs_sto.size()-1])
//     {
...
//     // }
//     // std::cout << _inputs_sto[0][_step-1] << "," << _inputs_sto[1][_step-1] << "," << _outputs_sto[_step-1] << std::endl;
//     //
//     // std::cout << _inputs_ptr[0][row_index-offset] << "," << _inputs_ptr[1][row_index-offset] << "," << std::abs(_values_ptr[0][row_index-offset]) << std::endl;
//     //
//     // std::cout << _step-1 << "," << (_inputs_sto[0].size()-1) << "," << (_inputs_sto[1].size()-1) << "," << (_outputs_sto.size()-1) << std::endl;
//
//     return _distributions[col_index]->quantile(getRand(_step));
//   } else if (_step > 2 && _step > _num_samplessub)
//   {
//     _subset = std::floor(_step / _num_samplessub);
//     if (col_index == 0 && std::abs(_values_ptr[0][row_index-offset]) != _outputs_sto[_outputs_sto.size()-1])
//     {
//       _count_max = std::floor(1 / _subset_probability);
//       if (_subset > (std::floor((_step-1) / _num_samplessub)))
//       {
//         _ind_sto = -1;
//         _count = 1000000;
//         _output_sorted = SubsetSim::sortOUTPUT(_outputs_sto, _num_samplessub, _subset, _subset_probability);
//         for (dof_id_type j = 0; j < _distributions.size(); ++j)
//         {
//           _inputs_sorted[j].resize(std::floor(_num_samplessub * _subset_probability));
//           _inputs_sorted[j] = SubsetSim::sortINPUT(_inputs_sto[j], _outputs_sto, _num_samplessub, _subset, _subset_probability);
//         }
//         _output_limits.push_back(SubsetSim::computeMIN(_output_sorted));
//       }
...
//         // _outputs_sto[(row_index-offset) * getParam<dof_id_type>("num_rows") + (_step-1)] = _outputs_sto[(row_index-offset) * getParam<dof_id_type>("num_rows") + (_step-2)];
//       } else
//       {
//         for (dof_id_type j = 0; j < _distributions.size(); ++j)
//           _inputs_sto[j].push_back(_inputs_sto[j][(row_index-offset) * getParam<dof_id_type>("num_rows") + (_step-2)]);
//         _outputs_sto.push_back(_outputs_sto[(row_index-offset) * getParam<dof_id_type>("num_rows") + (_step-2)]);
//       }
//       // std::vector<Real> K;
//       // K[1] = 1;
...
//       {
//         for (dof_id_type k = 0; k < _distributions.size(); ++k)
//           _markov_seed[k] = _inputs_sto[k][(row_index-offset) * getParam<dof_id_type>("num_rows") + (_step-1)];
//       }
//       ++_count;
//
//       Real rv;
...
//       {
//         rv = Normal::quantile(getRand(_step), _markov_seed[i], 0.15);
...
//         std::cout << _inputs_sto[0][i] << "," << _inputs_sto[1][i] << "," << _outputs_sto[i] << std::endl;
//       }
//       std::cout << _inputs_sto[0].size() << std::endl;
//     }
//
...
SubsetSim::computeSample(dof_id_type row_index, dof_id_type col_index)
{
  ...
  {
    _subset = std::floor(_step / _num_samplessub);
    if (col_index == 0 && _check_even != _step) // std::abs(_values_ptr[0][row_index-offset]) != _outputs_sto[_outputs_sto.size()-1]
    {
      for (dof_id_type j = 0; j < _distributions.size(); ++j)
        _inputs_sto[j].push_back(_inputs_ptr[j][row_index-offset]);
      _outputs_sto.push_back(std::abs(_values_ptr[0][row_index-offset]));
  ...
  } else if (_step > 2 && _step > _num_samplessub)
  {
    ...
    if (col_index == 0 && _check_even != _step) // std::abs(_values_ptr[0][row_index-offset]) != _outputs_sto[_outputs_sto.size()-1]
    {
      ...
      // std::cout << (std::floor((_step-2) / _num_samplessub)) << std::endl;
