...
AIS::computeSample(dof_id_type /*row_index*/, dof_id_type col_index)
{
  ...
  if (_step <= _num_samples_train)
  {
    ...
    _check_step = _step;
    _prev_value[col_index] = Normal::quantile(getRand(_step+col_index+1), _inputs_sto[col_index][_inputs_sto[col_index].size()-1], _proposal_std[col_index]);
    // std::cout << "Val is " << _prev_value[col_index] << std::endl;
