...
registerMooseObjectReplaced("StochasticToolsApp", AIS, "07/01/2020 00:00", MonteCarlo);
...
AIS::validParams()
{
  ...
  params.addRequiredParam<std::vector<ReporterName>>("inputs_reporter",
                                                     "Reporter with input parameters.");
    ...
    _perf_compute_sample(registerTimedSection("computeSample", 4))
{
  ...
  /* Adaptive Importance Sampling (AIS) relies on a Markov Chain Monte Carlo (MCMC) algorithm.
     ...
     main App's executioner would control the total number of samples. Therefore, the `num_rows`
     parameter typically used by exisiting non-MCMC samplers to set the total number of samples
...
AIS::computeSample(dof_id_type /*row_index*/, dof_id_type col_index)
{
  ...
  if (_step <= _num_samples_train)
  {
    ...
    if (_step > 1 && col_index == 0 && _check_step != _step)
    {
      for (dof_id_type j = 0; j < _distributions.size(); ++j)
        _prev_value[j] = Normal::quantile(
            _distributions[j]->cdf(getReporterValueByName<Real>(_inputs_names[j])), 0, 1);
      _acceptance_ratio = 0.0;
      for (dof_id_type i = 0; i < _distributions.size(); ++i)
        _acceptance_ratio += std::log(Normal::pdf(_prev_value[i], 0, 1)) -
                             std::log(Normal::pdf(_inputs_sto[i][_inputs_sto[i].size() - 1], 0, 1));
    ...
    _prev_value[col_index] =
        Normal::quantile(getRand(_step),
                         _inputs_sto[col_index][_inputs_sto[col_index].size() - 1],
                         _proposal_std[col_index]);
    // std::cout<< "Here " << _prev_value[col_index] << std::endl;
  ...
  else if (_step == _num_samples_train + 1)
  {
    ...
    if (col_index == 0 && _check_step != _step)
    {
      for (dof_id_type i = 0; i < _distributions.size(); ++i)
      {
        _mean_sto[i] = AdaptiveMonteCarloUtils::computeMEAN(_inputs_sto[i]);
        _std_sto[i] = AdaptiveMonteCarloUtils::computeSTD(_inputs_sto[i], 0);
        _prev_value[i] =
            (Normal::quantile(getRand(_step), _mean_sto[i], _std_factor * _std_sto[i]));
      }
    }
    _check_step = _step;
    return _distributions[col_index]->quantile(Normal::cdf(_prev_value[col_index], 0, 1));
