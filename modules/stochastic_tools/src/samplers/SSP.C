...
registerMooseObjectAliased("StochasticToolsApp", SSP, "SSP");
...
SSP::SSP(const InputParameters & parameters)
...
// Real
// SSP::computeSample(dof_id_type /*row_index*/, dof_id_type col_index)
// {
...
SSP::computeSample(dof_id_type row_index, dof_id_type col_index)
{
  ...
  {
    for (dof_id_type j = 0; j < _distributions.size(); ++j)
      _inputs_sto[j].push_back(Normal::quantile(_distributions[j]->cdf(getReporterValueByName<std::vector<Real>>(_inputs_names[j])[row_index]),0,1));
    _outputs_sto.push_back(getReporterValue<std::vector<Real>>("output_reporter")[row_index]);
    // for (dof_id_type ss = 0; ss < getNumberOfRows(); ++ss)
    // {
    //   for (dof_id_type j = 0; j < _distributions.size(); ++j)
    //     _inputs_sto[j].push_back(Normal::quantile(_distributions[j]->cdf(getReporterValueByName<std::vector<Real>>(_inputs_names[j])[ss]),0,1));
    //   _outputs_sto.push_back(getReporterValue<std::vector<Real>>("output_reporter")[ss]);
    // }
    // for (dof_id_type j = 0; j < _distributions.size(); ++j)
    // {
    //   for (dof_id_type ss = 0; ss < getNumberOfRows(); ++ss)
    //     _inputs_sto[j].push_back(Normal::quantile(_distributions[j]->cdf(getReporterValueByName<std::vector<Real>>(_inputs_names[j])[ss]),0,1));
    // }
    // _outputs_sto.push_back(getReporterValue<std::vector<Real>>("output_reporter")); // (_use_absolute_value) ? std::abs(getReporterValue<std::vector<Real>>("output_reporter")) : getReporterValue<std::vector<Real>>("output_reporter")
  }
  ...
  //     for (dof_id_type i = 0; i < _distributions.size(); ++i)
  //     {
  //       rv = _markov_seed[i] + (2 * getRand(_step+i+_distributions.size()) - 1) * 1.0; // _proposal_std[i]
