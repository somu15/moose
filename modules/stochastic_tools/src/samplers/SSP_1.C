...
registerMooseObjectAliased("StochasticToolsApp", SSP_1, "SSP_1");
...
SSP_1::SSP_1(const InputParameters & parameters)
    ...
    _perf_compute_sample(registerTimedSection("computeSample", 4))
{
  ...
  _subset = 0;
  _check_even = 0;
  setNumberOfRandomSeeds(700000);
  _proposal_std.resize(_distributions.size());
}

Real
SSP_1::computeSample(dof_id_type row_index, dof_id_type col_index)
{
  ...
  if (_step <= (_num_samplessub / n_processors()))
  {
    ...
    _check_even = _step;
    return _distributions[col_index]->quantile(getRand(_step)); // _step+col_index+processor_id()
  } else
  {
    ...
    if (col_index == 0 && _check_even != _step)
    {
      ...
      for (dof_id_type jj = 0; jj < n_processors(); ++jj)
      {
        for (dof_id_type i = 0; i < _distributions.size(); ++i)
        {
          ...
          rv = Normal::quantile(getRand(_step),_markov_seed[i][jj],1.0); // _step+i+_distributions.size()+processor_id()
          _acceptance_ratio = std::log(Normal::pdf(rv, 0, 1)) - std::log(Normal::pdf(_markov_seed[i][jj], 0, 1));

          if (std::isnan(_acceptance_ratio) == 1)
            _acceptance_ratio = 1;
