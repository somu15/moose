#include "ADElderContinuity.h"

registerMooseObject("PorousFlowApp", ADElderContinuity);

InputParameters
ADElderContinuity::validParams()
{
  InputParameters params = ADKernel::validParams();
  params.addClassDescription("Density-dependent Darcy continuity equation for the Elder "
...
ADElderContinuity::ADElderContinuity(const InputParameters & parameters)
  : ADKernel(parameters),
...
ADReal
ADElderContinuity::rho() const
{
  return _rho0 + _beta * _c[_qp];
}

ADRealVectorValue
ADElderContinuity::darcyVelocity() const
{
...
ADReal
ADElderContinuity::computeQpResidual()
{
